...
from stockeasy.collector.core.logger import LogContext, LoggerMixin, log_scheduler_job
...
class SchedulerService(LoggerMixin):
    ...
    def __init__(self, data_collector=None, cache_manager=None):
        ...
        self._job_stats = {"total_jobs": 0, "successful_jobs": 0, "failed_jobs": 0, "last_stock_update": None, "last_etf_update": None}
    ...
    async def _update_stock_list_job(self) -> None:
        ...
        try:
            ...
            if self.cache_manager:
                ...
                # 일반 종목 리스트 캐시 저장
                cache_data = {"data": stock_list, "metadata": {"updated_at": datetime.now().isoformat(), "total_count": len(stock_list), "source": "scheduler_update"}}
                await self.cache_manager.set_cache(
                    "all_stock_list",
                    cache_data,
                    ttl=86400 * 3,  # 3일 저장. 업데이트는 매일 아침 7시30분. 혹시라도 키움이 먹통됐을때를 대비.
                )

                # stockai용 종목 리스트 캐시 저장
                cache_data_stockai = {
                    "data": stock_list_for_stockai,
                    "metadata": {"updated_at": datetime.now().isoformat(), "total_count": len(stock_list_for_stockai), "source": "scheduler_update"},
                }
                await self.cache_manager.set_cache(
    ...
    async def _update_today_chart_data_job(self) -> None:
        ...
        try:
            ...
            logger.info(
                f"[스케줄러] 업데이트 통계 - 전체: {result.get('total_stocks', 0)}, 성공: {result.get('updated_stocks', 0)}, 수정주가 포함: {result.get('with_adjustment', 0)}, 오류: {result.get('errors', 0)}"
            )
    ...
    def get_job_stats(self) -> Dict[str, Any]:
        ...
        return {
            **self._job_stats,
            ...
            "scheduled_jobs": [
                {"id": job.id, "name": job.name, "next_run_time": job.next_run_time.isoformat() if job.next_run_time else None} for job in self.scheduler.get_jobs()
            ],
        }
    ...
    def get_scheduler_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"is_running": self._is_running, "error": str(e), "current_time": datetime.now().isoformat()}
