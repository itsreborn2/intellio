...
async def process_reports(db: AsyncSession, company_code: str, reports_dir: str, test_file: str = None, skip_annual_first: bool = True) -> bool:
    ...
    for i, file_name in enumerate(report_files):
        ...
        if test_mode and test_file not in file_name:
            continue
        if test_file:
        ...
        try:
            ...
            try:
                result = await service.process_financial_summary(
                    company_code=company_code, report_file_path=file_path, report_type=report_type, report_year=year, report_quarter=quarter
                )
...
async def process_one_company(company_code: str, reports_dir: str, test_file: str = None, skip_annual_first: bool = True):
    ...
    try:
        ...
        result = await process_reports(db=db, company_code=company_code, reports_dir=reports_dir, test_file=test_file, skip_annual_first=skip_annual_first)
        if result:
...
async def process_companies(code_list: list, reports_dir: str, test_file: str = None, skip_annual_first: bool = True, batch_size: int = 3):
    """
    ...
    for i in range(0, total_companies, batch_size):
        ...
        logger.info(
            f"배치 처리 시작: {i // batch_size + 1}/{(total_companies + batch_size - 1) // batch_size}, 회사 수: {len(batch)}개 ({i + 1}~{min(i + batch_size, total_companies)})"
        )
...
async def process_companies_each_files(file_list: list, reports_dir: str, batch_size: int = 3):
    ...
    for i in range(0, total_files, batch_size):
        ...
        logger.info(f"배치 처리 시작: {i // batch_size + 1}/{(total_files + batch_size - 1) // batch_size}, 회사 수: {len(batch)}개 ({i + 1}~{min(i + batch_size, total_files)})")
    ...
    try:
        ...
        if args.company and args.company == "all":
            ...
            for current_index in range(start_index, end_index, window_size):
                ...
                await process_companies_each_files(file_list=tmp_list, reports_dir=args.dir, batch_size=args.batch_size)
            ...
            await process_companies(code_list=code_list, reports_dir=args.dir, test_file=test_file, skip_annual_first=not args.process_all, batch_size=args.batch_size)
