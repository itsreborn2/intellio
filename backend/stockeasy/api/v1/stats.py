...
async def _fetch_popular_stocks_from_db(hours: int, limit: int, db: AsyncSession) -> List[Dict]:
    """
    ...
    # SQL 쿼리 구성
    query = (
        select(StockChatSession.stock_code, StockChatSession.stock_name, func.count().label("query_count"))
        .where(
            and_(
    ...
    for idx, row in enumerate(rows, 1):
        stock_item = {"stock_code": row.stock_code, "stock_name": row.stock_name, "query_count": row.query_count, "rank": idx}
        stocks.append(stock_item)

    return stocks


def _calculate_rank_changes(current_stocks: List[Dict], previous_stocks: List[Dict]) -> List[StockPopularityItem]:
    """
    순위 변동을 계산합니다.
    ...
    """
    # 이전 기간 종목별 순위 매핑
    previous_rank_map = {stock["stock_code"]: stock["rank"] for stock in previous_stocks}
    ...
    for current_stock in current_stocks:
        ...
        if previous_rank is None:
            # 신규 진입
            rank_change = RankChange(change_type=RankChangeType.NEW, change_value=0, previous_rank=None)
        else:
            ...
            rank_change = RankChange(change_type=change_type, change_value=change_value, previous_rank=previous_rank)

        # StockPopularityItem 생성
        stock_item = StockPopularityItem(
            stock_code=current_stock["stock_code"], stock_name=current_stock["stock_name"], query_count=current_stock["query_count"], rank=current_rank, rank_change=rank_change
        )
...
async def get_popular_stocks(
    limit: int = Query(20, ge=1, le=100, description="반환할 종목 수 (1~100개)"), db: AsyncSession = Depends(get_db_async), current_session: Session = Depends(get_current_session)
    ...
    try:
        ...
        # 응답 데이터 구성
        data_24h = PopularStocksPeriodData(stocks=stocks_24h_with_changes, period_hours=24, total_count=len(stocks_24h_with_changes), from_cache=from_cache_24h)
        ...
        return PopularStocksResponse(ok=True, status_message="인기 종목 조회 완료 (순위 변동 포함)", data_24h=data_24h, data_7d=data_7d)
...
async def get_session_count(
    ...
    try:
        ...
        # 총 세션 개수 쿼리
        total_query = select(func.count()).select_from(StockChatSession).where(StockChatSession.created_at >= start_time)

        # 종목이 포함된 세션 개수 쿼리
        with_stock_query = (
            select(func.count())
            .select_from(StockChatSession)
            .where(and_(StockChatSession.created_at >= start_time, StockChatSession.stock_code.is_not(None), StockChatSession.stock_code != ""))
        )
        ...
        logger.error(error_message, exc_info=True)
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="세션 개수 조회 중 오류가 발생했습니다. 잠시 후 다시 시도해주세요.")
