...
class AgentLLM:
    ...
    def _create_llm_with_fallback(self, streaming: bool = False, callback_handler: Optional[BaseCallbackHandler] = None) -> BaseChatModel:
        """
        ...
        if not self.fallback_settings.get("enabled", False):
            return LLMFactory.create_llm_from_config(self.llm_config, streaming=streaming, callback_handler=callback_handler)
        ...
        try:
            ...
            for retry in range(min(max_retries, len(fallback_providers))):
                try:
                    ...
                    # 폴백으로 LLM 생성
                    return LLMFactory.create_llm_from_config(fallback_config, streaming=streaming, callback_handler=callback_handler)
                except Exception as e:
                    # 폴백 실패 로그
    ...
    def invoke_with_fallback(self, *args, **kwargs) -> Any:
        ...
        if not self.fallback_settings.get("enabled", False):
            if use_token_tracking:
                # 토큰 추적 컨텍스트 생성
                with track_token_usage(user_id=user_id, project_type=project_type, token_type="llm", model_name=self.get_model_name(), db_getter=db) as tracker:
                    # LLM 호출
                    ...
                    if usage_info:
                        # 추출된 토큰 정보 사용
                        tracker.add_tokens(prompt_tokens=usage_info["prompt_tokens"], completion_tokens=usage_info["completion_tokens"], total_tokens=usage_info["total_tokens"])
        ...
        for provider_idx, provider_config in enumerate(providers_to_try):
            try:
                ...
                if use_token_tracking:
                    ...
                    with track_token_usage(user_id=user_id, project_type=project_type, token_type="llm", model_name=model_name, db_getter=db) as tracker:
                        # 필요시 LLM 설정 변경
                        if provider_idx > 0:
                            # 임시 폴백 LLM 설정
                        ...
                        if usage_info:
                            # 추출된 토큰 정보 사용
                            tracker.add_tokens(
                                prompt_tokens=usage_info["prompt_tokens"], completion_tokens=usage_info["completion_tokens"], total_tokens=usage_info["total_tokens"]
                            )
        ...
        if last_exception:
            ...
            raise RuntimeError("모든 LLM 호출 시도가 알 수 없는 이유로 실패했습니다.")
    ...
    async def ainvoke_with_fallback(self, *args, **kwargs) -> Any:
        ...
        if not self.fallback_settings.get("enabled", False):
            if use_token_tracking:
                # 토큰 추적 컨텍스트 생성
                async with track_token_usage(user_id=user_id, project_type=project_type, token_type="llm", model_name=self.get_model_name(), db_getter=db) as tracker:
                    # 비동기 LLM 호출
        ...
        for provider_idx, provider_config in enumerate(providers_to_try):
            try:
                ...
                if use_token_tracking:
                    ...
                    async with track_token_usage(user_id=user_id, project_type=project_type, token_type="llm", model_name=model_name, db_getter=db) as tracker:
                        # 필요시 LLM 설정 변경
                        if provider_idx > 0:
                            # 임시 폴백 LLM 설정
                            self.llm_config = provider_config
                        ...
                        # 2. _message 속성에서 확인 (구조화된 출력의 경우)
                            ...
                            logger.info("[토큰 추적][비동기] _message 속성에서 usage_metadata 찾음")
                            usage_info = {
                        ...
                        # 3. underlying_response 속성에서 확인
                            ...
                            logger.info("[토큰 추적][비동기] underlying_response 속성에서 usage_metadata 찾음")
                            usage_info = {
                        ...
                        # 4. _raw_response 속성에서 확인
                            ...
                            logger.info("[토큰 추적][비동기] _raw_response 속성에서 usage_metadata 찾음")
                            usage_info = {
                        ...
                        # 5. _original_message 속성에서 확인 (구조화된 출력 개선)
                            ...
                            logger.info("[토큰 추적][비동기] _original_message 속성에서 usage_metadata 찾음")
                            usage_info = {
        ...
        if last_exception:
            ...
            raise RuntimeError("모든 비동기 LLM 호출 시도가 알 수 없는 이유로 실패했습니다.")
    ...
    def with_structured_output(self, schema, **kwargs):
        ...
        class StructuredOutputAgentLLM:
            ...
            def invoke(self, *args, **kwargs):
                ...
                logger.info("[구조화된 출력][invoke] 원본 응답 획득을 위한 LLM 호출")

                raw_response = self.agent_llm.invoke_with_fallback(*args, user_id=user_id, project_type=project_type, db=db, **kwargs)
            ...
            if use_token_tracking:
                async with track_token_usage(user_id=user_id, project_type=project_type, token_type="llm", model_name=self.get_model_name(), db_getter=db) as tracker:
                    ...
                    if token_metadata_found:
                        ...
                        tracker.add_tokens(prompt_tokens=total_prompt_tokens, completion_tokens=total_completion_tokens, total_tokens=total_tokens)
                    else:
                        logger.warning("[스트리밍][토큰 추적] 메타데이터를 찾을 수 없습니다.")
            else:
                # 토큰 추적 없이 스트리밍
                async for chunk in llm.astream(input, **kwargs):
                    yield chunk
