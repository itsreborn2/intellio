...
class StockDividend(Base):
    ...
    # 배당 정보
    ex_dividend_date = Column(Date, comment="배당락일")
    record_date = Column(Date, comment="배당기준일")
    payment_date = Column(Date, comment="배당지급일")
    dividend_amount = Column(Numeric(10, 2), comment="주당 배당금")
    dividend_yield = Column(Numeric(5, 2), comment="배당수익률(%)")
    
    # 배당 종류
    dividend_type = Column(String(20), comment="배당 종류 (현금, 주식, 혼합)")
    year = Column(Integer, index=True, comment="배당 연도")
    quarter = Column(Integer, comment="분기 (1,2,3,4)")
    
    created_at = Column(DateTime, default=datetime.utcnow, comment="생성일시")
    
    __table_args__ = (
        Index('idx_dividend_symbol_year', 'symbol', 'year'),
...
class StockSplit(Base):
    ...
    # 분할/합병 정보
    split_date = Column(Date, nullable=False, comment="분할/합병일")
    split_ratio = Column(Numeric(10, 6), comment="분할/합병 비율")
    split_type = Column(String(20), comment="분할/합병 구분")
    
    # 분할 전후 주식수
    before_shares = Column(BigInteger, comment="분할 전 주식수")
    after_shares = Column(BigInteger, comment="분할 후 주식수")
    
    description = Column(Text, comment="분할/합병 설명")
    created_at = Column(DateTime, default=datetime.utcnow, comment="생성일시")
