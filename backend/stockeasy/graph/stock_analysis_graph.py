...
class StockAnalysisGraph:
    ...
    def __init__(self, agents: Dict[str, BaseAgent] = None):
        ...
        self.callbacks = {"agent_start": [], "agent_end": [], "graph_start": [], "graph_end": []}
    ...
    def clear_callbacks(self, event_type: Optional[str] = None):
        ...
        if event_type:
            ...
            for event_type in self.callbacks:
                self.callbacks[event_type] = []
            logger.info("모든 이벤트의 콜백 함수가 제거됨")
    ...
    def _build_graph(self, db: AsyncSession = None):
        ...
        # 새로운 흐름 정의: session_manager -> technical_analyzer -> question_analyzer -> orchestrator -> parallel_search -> knowledge_integrator
        ...
        def question_analyzer_router(state: AgentState) -> str:
            ...
            if is_conversation_closing:
                logger.info("대화 마무리로 에이전트를 종료합니다. END")
            ...
            if is_different_stock and stock_relation == "다른종목":
                logger.info("다른종목에 관란 질문이므로,  에이전트를 종료합니다. END")
            ...
            # if requires_context:
            if is_follow_up:
                logger.info("후속질문으로 context_response 에이전트로 라우팅합니다.")
                # 처리 상태 업데이트
                if "processing_status" not in state:
                    state["processing_status"] = {}
                state["processing_status"]["question_analyzer"] = "completed"
            ...
            logger.info("일반 질문으로 orchestrator 에이전트로 라우팅합니다.")
            return "orchestrator"
        ...
        workflow.add_conditional_edges("orchestrator", orchestrator_router, {"parallel_search": "parallel_search", "fallback_manager": "fallback_manager"})
        ...
        def parallel_search_router(state: AgentState) -> str:
            ...
            if not parallel_search_executed and not search_completed:
                ...
                logger.warning(
                    f"실행 상태: telegram={telegram_status}, report={report_status}, financial={financial_status}, industry={industry_status}, confidential={confidential_status}, revenue_breakdown={revenue_breakdown_status}"
                )
                return "fallback_manager"
        ...
        workflow.add_conditional_edges("parallel_search", parallel_search_router, {"knowledge_integrator": "knowledge_integrator", "fallback_manager": "fallback_manager"})

        # 통합 및 요약 에이전트 이후의 흐름
        workflow.add_conditional_edges("knowledge_integrator", self._determine_next_agent, {"summarizer": "summarizer", "response_formatter": "response_formatter", END: END})

        workflow.add_conditional_edges("summarizer", self._determine_next_agent, {"response_formatter": "response_formatter", END: END})
    ...
    def _create_wrapped_process(self, agent_name: str, original_process):
        ...
        async def wrapped_process(state: Dict[str, Any]) -> Dict[str, Any]:
            ...
            async def analyze_objects(phase):
                ...
                try:
                    ...
                    async def write_object_types_csv():
                        ...
                        await loop.run_in_executor(None, lambda: _write_object_types_to_csv(csv_path, file_exists, timestamp, session_id, agent_name, phase, most_common))
                    ...
                    def _write_object_types_to_csv(path, exists, timestamp, session_id, agent_name, phase, data):
                        with open(path, "a", newline="") as csvfile:
                            ...
                            for type_name, count in data:
                                writer.writerow({"timestamp": timestamp, "session_id": session_id, "agent": agent_name, "phase": phase, "object_type": type_name, "count": count})
                    ...
                    if agent_name == "parallel_search":
                        ...
                        if "retrieved_data" in state:
                            ...
                            async def write_retrieved_data_csv():
                                # CSV 파일에 데이터 추가 (이벤트 루프 차단 방지)
                                loop = asyncio.get_event_loop()
                                await loop.run_in_executor(None, lambda: _write_retrieved_data_to_csv(retrieved_data_path, retrieved_data))
                            ...
                            def _write_retrieved_data_to_csv(path, data):
                                with open(path, "w", newline="") as csvfile:
                                    ...
                                    for key, value in data.items():
                                        ...
                                        writer.writerow({"key": key, "type": type(value).__name__, "items_count": items_count, "size_kb": size_kb})
                        ...
                        if any(t[0] == "dict" and t[1] > 10000 for t in most_common):
                            ...
                            async def write_big_dicts_csv():
                                # CSV 파일에 데이터 추가 (이벤트 루프 차단 방지)
                                loop = asyncio.get_event_loop()
                                await loop.run_in_executor(None, lambda: _write_big_dicts_to_csv(big_dicts_path, big_dicts))
                            ...
                            def _write_big_dicts_to_csv(path, big_dicts_data):
                                with open(path, "w", newline="") as csvfile:
                                    ...
                                    for i, (size, d) in enumerate(big_dicts_data):
                                        ...
                                        writer.writerow(
                                            {"index": i + 1, "size": size, "sample_keys": str(keys), "first_value_type": first_value_type, "first_value_length": first_value_length}
                                        )
                    ...
                    if agent_name in ["parallel_search", "knowledge_integrator", "summarizer", "response_formatter"]:
                        ...
                        try:
                            ...
                            try:
                                ...
                                logger.warning("[객체분석-경고] objgraph 라이브러리가 설치되지 않아 그래프 생성을 건너뜁니다. 설치 명령: pip install objgraph")
                                has_objgraph = False
                            ...
                            if has_objgraph:
                                ...
                                async def create_graph():
                                    try:
                                        # 그래프 생성 (이벤트 루프 차단 방지)
                                        loop = asyncio.get_event_loop()
                                        await loop.run_in_executor(None, lambda: _create_object_graph(biggest_type, str(graph_path)))
                                ...
                                def _create_object_graph(obj_type, path):
                                    try:
                                        ...
                                        if sample_objs:
                                            ...
                                            try:
                                                objgraph.show_backrefs(sample_objs, filename=path, max_depth=5, too_many=10, filter=lambda x: not isinstance(x, type))
                                                ...
                                                if "graphviz" in str(graphviz_e).lower() or "dot" in str(graphviz_e).lower() or "executable not found" in str(graphviz_e).lower():
                                                    logger.warning("[객체분석-그래프] Graphviz가 설치되지 않아 PNG 그래프 대신 텍스트 참조 정보를 저장합니다.")
                                        ...
                                        if "graphviz" in str(create_e).lower() or "dot" in str(create_e).lower():
                                            logger.warning("[객체분석-그래프] Graphviz가 설치되지 않은 것 같습니다. 설치 가이드: https://graphviz.org/download/")
                                        # 간단한 텍스트 요약이라도 저장
                                        try:
            ...
            async def log_memory_to_csv(phase, memory_data):
                ...
                try:
                    ...
                    async def write_memory_csv():
                        # CSV 파일에 데이터 추가 (이벤트 루프 차단 방지)
                        loop = asyncio.get_event_loop()
                        await loop.run_in_executor(None, lambda: _write_memory_to_csv(csv_path, file_exists, timestamp, session_id, agent_name, phase, memory_data))
            ...
            logger.info(
                f"[wrapped_process] {agent_name} 콜백 상태 확인: hasattr(self, '_streaming_callback')={hasattr(self, '_streaming_callback')}, self._streaming_callback={getattr(self, '_streaming_callback', None) is not None}, 'streaming_callback' in state={'streaming_callback' in state}"
            )

            # technical_analyzer 에이전트에는 preliminary_chart_callback 직접 전달
            if agent_name == "technical_analyzer" and hasattr(self, "_preliminary_chart_callback") and self._preliminary_chart_callback:
                state["send_preliminary_chart"] = self._preliminary_chart_callback
                logger.info(
                    f"[wrapped_process] {agent_name} 에이전트에 preliminary_chart_callback 직접 전달 완료: {self._preliminary_chart_callback.__name__ if hasattr(self._preliminary_chart_callback, '__name__') else '이름 없는 함수'}"
                )
            elif agent_name == "technical_analyzer":
                logger.warning(f"[wrapped_process] {agent_name} 에이전트용 preliminary_chart_callback이 설정되지 않았습니다.")
            elif hasattr(self, "_streaming_callback") and self._streaming_callback and "streaming_callback" not in state:
                state["streaming_callback"] = self._streaming_callback  # 클래스 멤버로 저장
                logger.info(
                    f"[wrapped_process] {agent_name} 에이전트에 스트리밍 콜백 함수 추가: {self._streaming_callback.__name__ if hasattr(self._streaming_callback, '__name__') else '이름 없는 함수'}"
                )
                ...
                logger.info(
                    f"[wrapped_process] {agent_name} 에이전트의 None 콜백을 전역 콜백으로 대체: {self._streaming_callback.__name__ if hasattr(self._streaming_callback, '__name__') else '이름 없는 함수'}"
                )
            elif "streaming_callback" in state:
                logger.info(
                    f"[wrapped_process] {agent_name} 에이전트에 이미 콜백 함수가 있음: {state['streaming_callback'].__name__ if hasattr(state['streaming_callback'], '__name__') else '이름 없는 함수' if state['streaming_callback'] else 'None'}"
                )
            else:
                logger.warning(f"[wrapped_process] {agent_name} 에이전트에 스트리밍 콜백을 설정할 수 없음")
            ...
            try:
                ...
                async def background_memory_work():
                    ...
                    try:
                        ...
                        logger.info(
                            f"[메모리체크-종료-백그라운드] {agent_name} - 실행시간: {execution_time:.2f}s, RSS: {post_memory['rss']:.2f}MB, VMS: {post_memory['vms']:.2f}MB, 상태크기: {post_memory['state_size']}KB"
                        )
                        logger.info(
                            f"[메모리변화-백그라운드] {agent_name} - RSS: {memory_diff['rss']:.2f}MB, VMS: {memory_diff['vms']:.2f}MB, 상태크기: {memory_diff['state_size']}KB"
                        )
                ...
                async def background_error_memory():
                    ...
                    try:
                        ...
                        logger.error(
                            f"[메모리체크-오류-백그라운드] {agent_name} - 실행시간: {execution_time:.2f}s, RSS: {error_memory['rss']:.2f}MB, VMS: {error_memory['vms']:.2f}MB, 상태크기: {error_memory['state_size']}KB"
                        )
                ...
                state["errors"].append({"agent": agent_name, "error": str(e), "type": type(e).__name__, "timestamp": datetime.now()})
    ...
    def _determine_next_agent(self, state: AgentState) -> str:
        ...
        processing_status = state.get("processing_status", {})
        executed_agents = [agent for agent, status in processing_status.items() if status in ["completed", "completed_with_default_plan", "completed_no_data"]]
    ...
    async def process_query(self, query: str, session_id: Optional[str] = None, stock_code: Optional[str] = None, stock_name: Optional[str] = None, **kwargs) -> Dict[str, Any]:
        """
        사용자 쿼리를 처리하는 메인 메서드
        ...
        try:
            ...
            # preliminary_chart 콜백 함수 추출 및 클래스 멤버로 저장
            preliminary_chart_callback = kwargs.get("preliminary_chart_callback")
            if preliminary_chart_callback and callable(preliminary_chart_callback):
                logger.info(
                    f"[process_query] preliminary_chart 콜백 함수가 전달되었습니다: {preliminary_chart_callback.__name__ if hasattr(preliminary_chart_callback, '__name__') else '이름 없는 함수'}"
                )
                self._preliminary_chart_callback = preliminary_chart_callback  # 클래스 멤버로 저장

            # 초기 상태 설정 (콜백 함수들을 제외한 다른 모든 키워드 인자 사용)
            kwargs_copy = {k: v for k, v in kwargs.items() if k not in ["streaming_callback", "preliminary_chart_callback"]}
                    ...
                    try:
                        if "fallback_manager" in self.agents:
                            fallback_state = {
                                ...
                                "error": str(kwargs.get("previous_error")),
                                "errors": [
                                    {"agent": "stock_analysis_graph", "error": str(kwargs.get("previous_error")), "type": "LangSmithTimestampError", "timestamp": datetime.now()}
                                ],
                            }
                ...
                try:
                    if "fallback_manager" in self.agents:
                        fallback_state = {
                            ...
                            "session_id": session_id or "error_session",
                            "error": str(e),
                            "errors": [{"agent": "stock_analysis_graph", "error": str(e), "type": "LangSmithTimestampError", "timestamp": datetime.now()}],
                        }
                        result = await self.agents["fallback_manager"].process(fallback_state)
                        return result
                except Exception as fallback_error:
            ...
            error_result = {
                "query": query,
                "errors": [{"agent": "stock_analysis_graph", "error": str(e), "type": type(e).__name__, "timestamp": datetime.now()}],
