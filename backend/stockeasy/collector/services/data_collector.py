...
# 공통 유틸리티 함수 import
from common.utils.util import safe_float, safe_float_or_none, safe_int, safe_int_or_none
from stockeasy.collector.core.config import get_settings
from stockeasy.collector.core.logger import LogContext, LoggerMixin
...
from stockeasy.collector.schemas.timescale_schemas import IntervalType, StockPriceCreate, SupplyDemandCreate
...
class DataCollectorService(LoggerMixin):
    ...
    def __init__(self, cache_manager: CacheManager):
        ...
        # 스케줄러 서비스
        self.scheduler_service = SchedulerService(data_collector=self, cache_manager=cache_manager)
    ...
    async def get_supply_demand_data(self, symbol: str, start_date: str, end_date: str = None, compressed: bool = False) -> Optional[SupplyDemandResponse]:
        ...
        try:
            ...
            supply_demand_data = await timescale_service.get_supply_demand_data(symbol=symbol, start_date=start_dt, end_date=end_dt)
            ...
            # 압축 형태 요청 시
            if compressed:
                return await self._create_compressed_supply_demand_response(supply_demand_data, symbol, symbol_name, start_date, end_date)

            # 기본 JSON 형태
            return await self._create_standard_supply_demand_response(supply_demand_data, symbol, symbol_name, start_date, end_date)

        except Exception as e:
            self.logger.error(f"수급 데이터 조회 실패 [{symbol}, {start_date}~{end_date}]: {e}")
            return None

    async def get_chart_data(self, symbol: str, period: str, interval: str, compressed: bool = False) -> Optional[ChartDataResponse]:
        ...
        try:
            ...
            candle_response = await timescale_service.get_candle_data(symbol=symbol, start_date=start_date, end_date=end_date, interval_type=interval_type)
            ...
            # 압축 형태 요청 시
            if compressed:
                return await self._create_compressed_chart_response(candle_response, symbol, symbol_name, period, interval)

            # 기본 JSON 형태
            return await self._create_standard_chart_response(candle_response, symbol, symbol_name, period, interval)

        except Exception as e:
            self.logger.error(f"차트 데이터 조회 실패 [{symbol}, {period}, {interval}]: {e}")
            return None

    async def _create_standard_chart_response(self, candle_response, symbol: str, symbol_name: str, period: str, interval: str) -> ChartDataResponse:
        ...
        return ChartDataResponse(symbol=symbol, name=symbol_name, period=period, interval=interval, data=chart_points, total_count=len(chart_points))

    async def _create_compressed_chart_response(self, candle_response, symbol: str, symbol_name: str, period: str, interval: str) -> CompressedChartDataResponse:
        """압축된 형태의 차트 응답 생성"""
        from datetime import timezone
        ...
        return CompressedChartDataResponse(symbol=symbol, name=symbol_name, period=period, interval=interval, schema=schema, data=compressed_data, total_count=len(compressed_data))
    ...
    async def get_market_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            is_market_hours = current_time.hour >= 9 and (current_time.hour < 15 or (current_time.hour == 15 and current_time.minute <= 30))
    ...
    async def get_market_indices(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            market_codes = [{"code": "001", "name": "KOSPI"}, {"code": "101", "name": "KOSDAQ"}]
            ...
            if indices_list:
                # Redis에 15분 TTL로 저장
                cache_data = {"indices": indices_list, "cached_at": datetime.now().isoformat()}
    ...
    async def get_collection_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "realtime_status": {"is_running": self._realtime_running, "active_tasks": len([t for t in self._collection_tasks if not t.done()])},
            }
    ...
    async def update_symbol_mappings(self) -> Dict[str, int]:
        ...
        try:
            ...
            return {"updated_stocks": self._stats["cached_symbols"], "updated_etfs": len(self.etf_crawler.get_major_etf_list())}
    ...
    async def get_all_stock_list_for_stockai(self) -> List[Dict[str, str]]:
        ...
        try:
            ...
            if stock_dict:
                # 메타데이터와 함께 저장
                cache_data = {"data": stock_dict, "metadata": {"updated_at": datetime.now().isoformat(), "total_count": len(stock_dict), "source": "kiwoom_api"}}
    ...
    async def force_refresh_stock_list(self) -> Dict[str, int]:
        ...
        try:
            ...
            # 캐시에 저장 (메타데이터 포함)
            cache_data = {"data": stock_list, "metadata": {"updated_at": datetime.now().isoformat(), "total_count": len(stock_list), "source": "kiwoom_api_force_refresh"}}
            await self.cache_manager.set_cache(
                "all_stock_list",
                cache_data,
                ttl=86400,  # 24시간
            ...
            cache_data_stockai = {
                ...
                "metadata": {"updated_at": datetime.now().isoformat(), "total_count": len(stock_list_for_stockai), "source": "kiwoom_api_force_refresh"},
            }
            await self.cache_manager.set_cache(
                "all_stock_list_for_stockai",
                cache_data_stockai,
            ...
            return {"total_stocks": len(stock_list), "update_time": int(datetime.now().timestamp())}
    ...
    async def get_all_last_update_times(self) -> Dict[str, Optional[datetime]]:
        ...
        """
        try:
            return {"stockai_update": await self.get_last_update_time("stockai"), "stock_update": await self.get_last_update_time("stock")}
    ...
    async def collect_all_stock_chart_data(self, months_back: int = 24, batch_size: int = 50, force_update: bool = False, progress_callback=None) -> Dict[str, Any]:
        """
        ...
        with LogContext(self.logger, f"전종목 일봉 차트 데이터 수집 ({months_back}개월) - 큐 기반" + (" [강제 업데이트]" if force_update else "")) as ctx:
            try:
                ...
                if force_update:
                    ...
                    ctx.log_progress("전체 주가 데이터 테이블 TRUNCATE 완료")
                ...
                # 공유 상태 관리
                stats = {"processed_stocks": 0, "success_stocks": 0, "error_stocks": 0, "total_records": 0, "api_finished": False}
                    ...
                    try:
                        for i in range(0, total_stocks, batch_size):
                            ...
                            try:
                                # 키움 API에서 차트 데이터 조회
                                if self.settings.KIWOOM_APP_KEY != "test_api_key":
                                    chart_data_dict = await self.kiwoom_client.get_multiple_chart_data(symbols, start_date_str, end_date_str)
                                else:
                                        ...
                                        try:
                                            ...
                                            if api_volume_change_percent is not None:
                                                if abs(api_volume_change_percent) > 999999.9999:
                                                    self.logger.warning(
                                                        f"종목 {symbol} 날짜 {chart_item.date}: volume_change_percent 값이 너무 큼 ({api_volume_change_percent}), 999999.9999로 제한"
                                                    )
                                ...
                                # 큐에 데이터 추가 (배치 정보 포함)
                                if stock_price_data:
                                    batch_info = {"data": stock_price_data, "symbols": symbols, "batch_num": i // batch_size + 1}
                    ...
                    try:
                        while True:
                            ...
                            try:
                                ...
                                for i in range(0, len(stock_price_data), max_batch_size):
                                    ...
                                    if force_update:
                                        ...
                                        await timescale_service.bulk_create_stock_prices(BulkStockPriceCreate(prices=sub_batch))
                                        ...
                                        await timescale_service.bulk_create_stock_prices_with_progress(sub_batch, batch_size=2000, progress_callback=None)
                                ...
                                if not force_update:
                                    ...
                                    try:
                                        calc_result = await timescale_service.batch_calculate_for_new_data(symbols=batch_symbols)
                            ...
                            if progress_callback:
                                ...
                                await progress_callback(stats["processed_stocks"], total_stocks, progress, stats["success_stocks"], stats["error_stocks"], stats["total_records"])
                ...
                ctx.log_progress(
                    f"전종목 차트 데이터 수집 완료 (큐 기반{'[강제 업데이트]' if force_update else ''}): {stats['success_stocks']}/{total_stocks} 종목, {stats['total_records']}건"
                )
                return result
    ...
    async def collect_all_supply_demand_data(self, months_back: int = 6, batch_size: int = 20, progress_callback=None) -> Dict[str, Any]:
        """
                    ...
                    try:
                        for i in range(0, total_stocks, batch_size):
                            ...
                            try:
                                # 키움 API에서 수급 데이터 조회
                                if self.settings.KIWOOM_APP_KEY != "test_api_key":
                                    supply_data_dict = await self.kiwoom_client.get_multiple_supply_demand_data(symbols, start_date_str, end_date_str)
                                else:
                                    ctx.logger.warning(f"키움 API 키가 설정되지 않아 배치 {i // batch_size + 1} 건너뜀")
                                    continue
                                ...
                                if supply_demand_data:
                                    batch_info = {"data": supply_demand_data, "symbols": symbols, "batch_num": i // batch_size + 1}
                                    await data_queue.put(batch_info)
                    ...
                    try:
                        while True:
                            ...
                            try:
                                ...
                                # TimescaleDB에 저장
                                await timescale_service.bulk_create_supply_demand_with_progress(supply_demand_data, batch_size=500, progress_callback=None)
    ...
    async def _create_standard_supply_demand_response(self, supply_demand_data, symbol: str, symbol_name: str, start_date: str, end_date: str) -> SupplyDemandResponse:
        ...
        return SupplyDemandResponse(symbol=symbol, name=symbol_name, start_date=start_date, end_date=end_date, data=supply_demand_points, total_count=len(supply_demand_points))

    async def _create_compressed_supply_demand_response(self, supply_demand_data, symbol: str, symbol_name: str, start_date: str, end_date: str) -> CompressedSupplyDemandResponse:
        ...
        return CompressedSupplyDemandResponse(
            symbol=symbol, name=symbol_name, start_date=start_date, end_date=end_date, schema=schema, data=compressed_data, total_count=len(compressed_data)
        )
        ...
        try:
            ...
            for i in range(0, len(all_symbols), batch_size):
                ...
                try:
                    ...
                    if self.settings.KIWOOM_APP_KEY != "test_api_key":
                        batch_data = await self.kiwoom_client.get_realtime_stock_prices_batch(batch_symbols, today_str)
                        ...
                        for symbol in batch_symbols:
                            try:
                                ...
                                if adjustment_info:
                                    ...
                                    if not scheduler_mode:
                                        self.logger.debug(
                                            f"종목 {symbol} 메모리에서 수정주가 정보 조회: type={adjustment_type}, ratio={adjustment_ratio}, event={adjustment_event}"
                                        )
            ...
            return {"message": error_msg, "total_stocks": 0, "updated_stocks": 0, "with_adjustment": 0, "errors": 1, "date": today_str, "status": "failed"}
        finally:
    ...
    async def update_today_supply_demand_data(self, batch_size: int = 50, scheduler_mode: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            stock_list = await self.get_all_stock_list_for_stockai()
            if not stock_list:
                return {"message": "종목 리스트가 비어있습니다", "total_stocks": 0, "updated_stocks": 0, "errors": 0, "status": "completed"}
            ...
            for i in range(0, len(all_symbols), batch_size):
                ...
                try:
                    ...
                    if self.settings.KIWOOM_APP_KEY != "test_api_key":
                        supply_data_dict = await self.kiwoom_client.get_multiple_supply_demand_data(batch_symbols, today_str, today_str)
            ...
            if scheduler_mode:
                ...
                self.logger.error(error_msg)
            return {"message": error_msg, "total_stocks": 0, "updated_stocks": 0, "errors": 1, "status": "failed"}
        finally:
            # 스케줄러 모드에서 변경한 로그 레벨 복원
            if scheduler_mode and original_log_level is not None and hasattr(self.kiwoom_client, "logger"):
                self.kiwoom_client.logger.setLevel(original_log_level)
    ...
    async def check_adjustment_prices_for_stockai(self, batch_size: int = 100) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"message": f"수정주가 체크 실패: {str(e)}", "total_stocks": 0, "checked_stocks": 0, "adjusted_stocks": 0, "errors": 1, "status": "failed"}
    ...
    def get_adjustment_check_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "cache_stats": {"total_cached_stocks": len(self._adjustment_data_cache["adjusted_stocks"]), "total_history_entries": len(self._adjustment_data_cache["check_history"])},
        }
    ...
    async def collect_sector_chart_data(
        ...
        try:
            ...
            if self.settings.KIWOOM_APP_KEY != "test_api_key":
                sector_data = await self.kiwoom_client.get_sector_chart_data(sector_code, start_date_str, end_date_str)
            else:
                self.logger.warning("키움 API 키가 설정되지 않아 더미 데이터 사용")
                sector_data = []
            ...
            for sector_item in sector_data:
                try:
                    ...
                    if len(stock_price_data) < 5:  # 처음 5개 데이터만 로깅
                        self.logger.info(
                            f"업종 {sector_symbol} {sector_item.dt} 가격 변환 예시: 시가 {sector_item.open_pric}->{open_price}, 고가 {sector_item.high_pric}->{high_price}, 저가 {sector_item.low_pric}->{low_price}, 종가 {sector_item.cur_prc}->{close_price}"
                        )
            ...
            if stock_price_data:
                ...
                if force_update:
                    ...
                    await timescale_service.delete_stock_prices_by_symbol_period(sector_symbol, start_date, end_date)
                ...
                await timescale_service.bulk_create_stock_prices_with_upsert(BulkStockPriceCreate(prices=stock_price_data))

                # 변동률 재계산 (업종 데이터용)
                self.logger.info(f"업종 {sector_symbol} 변동률 재계산 시작")
                await timescale_service.batch_calculate_for_new_data(symbols=[sector_symbol], start_date=start_date)
            ...
            if stock_price_data:
                ...
                self.logger.info(
                    f"업종 {sector_symbol} 가격 변환 및 UPSERT 완료 - 첫째: {first_data.time.strftime('%Y-%m-%d')} 종가 {first_data.close}, 마지막: {last_data.time.strftime('%Y-%m-%d')} 종가 {last_data.close}, 총 {len(stock_price_data)}건"
                )
            ...
            return {"message": f"업종 {sector_symbol} 차트 데이터 수집 실패: {str(e)}", "sector_symbol": sector_symbol, "total_records": 0, "status": "failed"}

    async def collect_all_sector_chart_data(self, months_back: int = 24, force_update: bool = False) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            for sector in sectors:
                ...
                result = await self.collect_sector_chart_data(sector_symbol=sector, months_back=months_back, force_update=force_update)
            ...
            return {"message": f"전체 업종 차트 데이터 수집 실패: {str(e)}", "total_sectors": 0, "successful_sectors": 0, "total_records": 0, "status": "failed"}
