...
def get_cached_vector_store_manager(embedding_model_type, namespace: str = None, project_name: str = None):
    ...
    if cache_key not in _global_vector_store_cache:
        ...
        # 새 인스턴스 생성
        vs_manager = VectorStoreManager(embedding_model_type=embedding_model_type, namespace=namespace, project_name=project_name)
...
def warm_up_vector_store_managers():
    ...
    try:
        ...
        common_configs = [
            ...
            {"embedding_model_type": EmbeddingModelType.OPENAI_3_LARGE, "namespace": settings.PINECONE_NAMESPACE_STOCKEASY_TELEGRAM, "project_name": "stockeasy"},
            {"embedding_model_type": EmbeddingModelType.OPENAI_3_LARGE, "namespace": settings.PINECONE_NAMESPACE_STOCKEASY_INDUSTRY, "project_name": "stockeasy"},
            {"embedding_model_type": EmbeddingModelType.OPENAI_3_LARGE, "namespace": settings.PINECONE_NAMESPACE_STOCKEASY_CONFIDENTIAL_NOTE, "project_name": "stockeasy"},
...
def get_vector_store_cache_info():
    ...
    """
    global _global_vector_store_cache, _cache_initialized
    return {"cache_size": len(_global_vector_store_cache), "cache_keys": list(_global_vector_store_cache.keys()), "initialized": _cache_initialized}
...
class AgentRegistry:
    ...
    def get_cached_vector_store_manager(self, embedding_model_type, namespace: str = None, project_name: str = None):
        ...
        Returns:
            캐시된 또는 새로 생성된 VectorStoreManager 인스턴스
        """
        return get_cached_vector_store_manager(embedding_model_type=embedding_model_type, namespace=namespace, project_name=project_name)
