...
export default function BreakoutFailChart() {
  // 차트 정보 상태 관리
  ...
  useEffect(() => {
    ...
    const loadAllData = async () => {
      ...
      try {
        // 1. 파일 목록 가져오기
        const fileListResponse = await fetch('/requestfile/breakoutchart/fail/file_list.json', { cache: 'no-store' });
        ...
        try {
          ...
          if (!breakoutResponse.ok) {
            console.warn(`breakout.csv 파일 로드 실패: ${breakoutResponse.status}`);
          } else {
              ...
              breakoutCsvData.data.forEach((row: any) => {
                ...
                if (stockCode && priceString) {
                  ...
                  if (!isNaN(price)) {
                    ...
                    // console.warn(`[BreakoutFailChart] Breakthrough Price 값 '${priceString}' (종목코드: ${stockCode})은 유효한 숫자가 아닙니다.`);
                  }
                }
              });
              // console.log('[BreakoutFailChart] 돌파 가격 맵:', breakthroughPriceMap);
            }
          }
        } catch (err) {
          console.warn('[BreakoutFailChart] breakout.csv 처리 중 오류:', err);
        }
        ...
        for (let i = 0; i < chartFiles.length; i++) {
          ...
          try {
            ...
            // 차트 데이터 로드
            const response = await fetch(`/requestfile/breakoutchart/fail/${fileName}`, { cache: 'no-store' });
            if (!response.ok) {
            ...
            // 등락률 및 RS 값 추출 - 마지막 행(최신 데이터)에서 가져옴
            let fileChangeRate = changeRate;
            let fileRSValue = rsValue;
            if (csvData.data && csvData.data.length > 0) {
              const lastRow = csvData.data[csvData.data.length - 1] as any;
              if (lastRow["등락률"]) {
                fileChangeRate = lastRow["등락률"];
                console.log(`추출된 등락률 (마지막 행, ${fileName}):`, fileChangeRate);
              }
              if (lastRow["RS"]) {
                fileRSValue = lastRow["RS"];
                console.log(`추출된 RS (마지막 행, ${fileName}):`, fileRSValue);
              }
            }

            // 차트 데이터 변환
            const chartData = await loadChartData(`/requestfile/breakoutchart/fail/${fileName}`);
            
            newChartInfos.push({
              code,
              name,
              market: marketValue,  // CSV에서 추출한 시장구분 사용
              changeRate: fileChangeRate,
              rsValue: fileRSValue,
              data: chartData,
              isLoading: false,
              error: '',
  ...
  const loadUpdateDate = async () => {
    try {
      // 파일 목록 가져오기
      const fileListResponse = await fetch('/requestfile/breakoutchart/fail/file_list.json', { 
        cache: 'no-store',
        method: 'HEAD'
      });
