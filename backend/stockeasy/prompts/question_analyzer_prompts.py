...
아래와 같은 목차로 생성합니다. 목차 번호는 기술적 분석 섹션의 순서에 따릅니다.
  1. 기술적 분석
   1.1 "주가 및 지지/저항" (추세선, 지지선, 저항선 분석)
   1.2 "추세추종 지표 분석" (RS, ADX, SuperTrend 등 - 우선 포함)
   1.3 "모멘텀 지표 분석" (RSI, 스토캐스틱, MACD 등 - 보조적 분석)
   1.4 "주가, 거래량, 수급 분석" (주가의 움직임을 거래량, 수급 관점에서 분석, 예상되는 추세/움직임에 대한 의견 적극 제시)
   1.5 "종합 분석" (종합적 매매 관점, 예상 시나리오 제시. 매매 전략은 제시금지.)
