...
def async_retry(retries: int = 3, delay: float = 1.0, backoff_factor: float = 2.0, exceptions: tuple = (Exception,)) -> Callable:
        ...
        async def wrapper(*args: Any, **kwargs: Any) -> Any:
            ...
            for attempt in range(retries + 1):
                try:
                    ...
                    last_exception = e
                    if attempt == retries:
                        logger.error(f"함수 {func.__name__} 실행 실패 (최대 재시도 횟수 초과)\n에러: {str(e)}\nArgs: {args}\nKwargs: {kwargs}", exc_info=True)
                        raise

                    logger.warning(f"함수 {func.__name__} 실행 실패 (시도 {attempt + 1}/{retries + 1})\n에러: {str(e)}\n대기 시간: {wait_time}초")
