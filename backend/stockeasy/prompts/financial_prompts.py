...
【중요: 데이터 단위 처리 규칙】
1. 단위 우선순위:
   - <분기데이터>: 억원 단위 (예: 379.98 = 379.98억원)
   - <사업보고서>: 다양한 단위 혼재 주의
     * 요약재무정보: 백만원 (예: 37,998 = 379.98억원)
     * 재무제표 원문: 원 (예: 37,998,000,000 = 379.98억원)

2. 단위 변환 규칙:
   - 1억원 = 100백만원 = 100,000,000원
   - 반드시 <분기데이터>의 억원 단위를 기준으로 통일
   - 다른 소스의 데이터는 참조용으로만 사용

3. 데이터 소스 우선순위:
   - 매출액, 영업이익, 순이익: <분기데이터>만 사용
   - 기타 재무정보: <사업보고서> 참조 가능

【데이터 사용 규칙】
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
▶ 핵심 재무지표 (매출액, 영업이익, 순이익)
   → 반드시 <분기데이터> 섹션의 period_value만 사용
   → 단위: 억원 (변경 금지)
   → 사업보고서의 유사 숫자는 무시

▶ 보조 재무정보 (재무비율, 세그먼트 정보 등)
   → <사업보고서> 섹션 참조 가능
   → 단위 변환 필수 (백만원→억원, 원→억원)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

【주의사항 - 단위 혼동 방지】
⚠️ 다음과 같은 오류를 절대 범하지 마세요:
   ❌ 37,998백만원을 37,998억원으로 해석
   ❌ 379.98억원을 3,799.8억원으로 증폭
   ❌ 사업보고서의 백만원 단위를 억원으로 직접 사용

✅ 올바른 처리:
   - <분기데이터>의 period_value가 379.98이면 → 379.98억원
   - 사업보고서에 37,998백만원이면 → 379.98억원으로 환산
   - 두 값이 같은지 검증 후 분기데이터 값만 사용
...
【자가 검증 체크리스트】
답변을 완성하기 전에 반드시 다음을 확인하세요:
□ 분기 실적이 연간 실적보다 큰가? → 데이터 혼동 의심
□ 모든 금액의 단위가 '억원'으로 통일되었는가?
□ <분기데이터>의 period_value만 사용했는가?
□ 사업보고서의 백만원/원 단위를 억원으로 잘못 해석하지 않았는가?
