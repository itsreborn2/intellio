...
class StockInfoService:
    ...
    async def read_krx_code(self):
        ...
        def safe_format_code(code):
            try:
                # 숫자형으로 변환 시도 후 포맷팅
                return f"{int(float(code)):06d}"
            except (ValueError, TypeError):
                # 변환 실패 시 원본 값을 문자열로 반환
                return str(code)

        krx.code = krx.code.map(safe_format_code)
        krx = krx[~krx["name"].str.contains("스팩")]
        # krx.to_csv(f"listed_company.csv",index=False,  encoding="utf-8-sig")
        df1 = pd.DataFrame({"code": ["KOSPI", "KOSDAQ"], "name": ["KOSPI", "KOSDAQ"], "업종": ["KOSPI", "KOSDAQ"]})
    ...
    async def update_all_stock_from_krx(self):
        ...
        try:
            ...
            krx_dict = {}
            for _, row in krx_data.iterrows():
                krx_dict[row["code"]] = {"name": row["name"]}
            ...
            for code, data in sheet_dict.items():
                if code in krx_dict and krx_dict[code]["name"] != data["name"]:
                    name_changes.append({"code": code, "old_name": data["name"], "new_name": krx_dict[code]["name"], "row": data["row"]})
            ...
            for code, data in sheet_dict.items():
                if code not in krx_dict:
                    delisted.append({"code": code, "name": data["name"], "row": data["row"]})
            ...
            for code, data in krx_dict.items():
                ...
                if code not in sheet_dict and code not in ["KOSPI", "KOSDAQ"]:
                    new_listings.append({"code": code, "name": data["name"]})
