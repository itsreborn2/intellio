...
async def get_all_stock_list_for_stockai(
    gzip_enabled: bool = Query(False, description="gzip 압축 사용 (더 작은 크기)"), data_collector: DataCollectorService = Depends(get_data_collector)
):
    """
    ...
    try:
        ...
        response_data = {
            ...
            "data": [[stock["code"], stock["name"], stock.get("market", "")] for stock in stock_list],
        }
...
async def get_all_stock_list(data_collector: DataCollectorService = Depends(get_data_collector)):
    """전체 종목 리스트 조회 (code, name만)"""
    try:
        stock_list = await data_collector.get_all_stock_list()
        return {"count": len(stock_list), "status": "success", "stocks": stock_list}
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"종목 리스트 조회 실패: {str(e)}")
...
async def search_stocks(
    ...
    try:
        ...
        return {"keyword": keyword, "stocks": results, "count": len(results), "status": "success"}
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"종목 검색 실패: {str(e)}")


@stock_router.get("/list/refresh")
async def refresh_stock_list(data_collector: DataCollectorService = Depends(get_data_collector)):
    """종목 리스트 강제 새로고침"""
    try:
        result = await data_collector.force_refresh_stock_list()
        return {"message": "종목 리스트 새로고침 완료", "result": result, "status": "success"}
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"종목 리스트 새로고침 실패: {str(e)}")


@stock_router.get("/price/{symbol}")
async def get_stock_price(symbol: str, data_collector: DataCollectorService = Depends(get_data_collector)):
    """실시간 주식 가격 조회"""
    try:
...
async def get_stock_info(symbol: str, data_collector: DataCollectorService = Depends(get_data_collector)):
    """종목 기본정보 조회 (키움 API)"""
    try:
        ...
        return {"symbol": symbol, "data": stock_info, "status": "success"}
...
async def get_supply_demand(
    ...
    try:
        ...
        supply_demand = await data_collector.get_supply_demand_data(symbol, start_date, end_date, compressed)
...
async def get_stock_chart(
    ...
    try:
        ...
        response_data = {"symbol": symbol, "period": period, "interval": interval, "compressed": compressed, "gzip_enabled": gzip_enabled, "data": chart_data, "status": "success"}
...
async def get_multiple_stock_charts(
    ...
    try:
        ...
        for symbol in symbol_list:
            try:
                chart_data = await data_collector.get_chart_data(symbol, period, interval, compressed)
                chart_results[symbol] = {"symbol": symbol, "data": chart_data, "status": "success"}
                logger.info(f"종목 {symbol} 차트 데이터 조회 성공")
            except Exception as e:
                logger.error(f"종목 {symbol} 차트 데이터 조회 실패: {e}")
                chart_results[symbol] = {"symbol": symbol, "data": None, "status": "failed", "error": str(e)}
...
@etf_router.get("/list")
async def get_etf_list(gzip_enabled: bool = Query(False, description="gzip 압축 사용 (더 작은 크기)"), data_collector: DataCollectorService = Depends(get_data_collector)):
    """
    ...
    try:
        ...
        response_data = {
            ...
            "status": "success",
            "headers": ["code", "name"],
            "data": [[etf.get("code", ""), etf.get("name", "")] for etf in etf_list],
        }
...
async def get_etf_components(
    etf_code: str, gzip_enabled: bool = Query(False, description="gzip 압축 사용 (더 작은 크기)"), data_collector: DataCollectorService = Depends(get_data_collector)
):
    """
    ETF 구성종목 조회 (pykrx)
    ...
    try:
        ...
        if components:
            ...
            # 표준 형태로 반환
            response_data = {"etf_code": etf_code, "components": components, "count": len(components), "compressed": False, "gzip_enabled": gzip_enabled, "status": "success"}
...
async def refresh_etf_components(etf_code: str, data_collector: DataCollectorService = Depends(get_data_collector)):
    """ETF 구성종목 갱신"""
    try:
        await data_collector.refresh_etf_components(etf_code)
        return {"etf_code": etf_code, "message": "ETF 구성종목 갱신 완료", "status": "success"}
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"ETF 구성종목 갱신 실패: {str(e)}")


@etf_router.post("/components/refresh-all")
async def refresh_all_etf_components(data_collector: DataCollectorService = Depends(get_data_collector)):
    """모든 주요 ETF 구성종목 일괄 갱신"""
    try:
        results = await data_collector.update_all_etf_components()
        total_components = sum(results.values())

        return {"message": "모든 ETF 구성종목 갱신 완료", "results": results, "total_etfs": len(results), "total_components": total_components, "status": "success"}
    except Exception as e:
...
async def get_market_status(data_collector: DataCollectorService = Depends(get_data_collector)):
    """시장 상태 조회"""
    try:
        market_status = await data_collector.get_market_status()
        return {"market_status": market_status, "status": "success"}
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"시장 상태 조회 실패: {str(e)}")


@market_router.get("/indices")
async def get_market_indices(data_collector: DataCollectorService = Depends(get_data_collector)):
    """주요 지수 조회"""
    try:
        indices = await data_collector.get_market_indices()
        return {"indices": indices, "status": "success"}
    except Exception as e:
...
async def start_scheduler(data_collector: DataCollectorService = Depends(get_data_collector)):
    """스케줄러 시작"""
    try:
        await data_collector.scheduler_service.start()
        return {"message": "스케줄러 시작됨", "status": "success"}
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"스케줄러 시작 실패: {str(e)}")


@admin_router.post("/scheduler/stop")
async def stop_scheduler(data_collector: DataCollectorService = Depends(get_data_collector)):
    """스케줄러 중지"""
    try:
        await data_collector.scheduler_service.shutdown()
        return {"message": "스케줄러 중지됨", "status": "success"}
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"스케줄러 중지 실패: {str(e)}")


@admin_router.get("/scheduler/stats")
async def get_scheduler_stats(data_collector: DataCollectorService = Depends(get_data_collector)):
    """스케줄러 통계 조회"""
    try:
        stats = data_collector.scheduler_service.get_job_stats()
        return {"scheduler_stats": stats, "status": "success"}
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"스케줄러 통계 조회 실패: {str(e)}")


@admin_router.post("/scheduler/trigger/stocks")
async def trigger_stock_update(data_collector: DataCollectorService = Depends(get_data_collector)):
    """즉시 종목 리스트 업데이트 실행"""
    try:
        await data_collector.scheduler_service.trigger_stock_update_now()
        return {"message": "종목 리스트 업데이트 실행 완료", "status": "success"}
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"종목 리스트 업데이트 실행 실패: {str(e)}")


@admin_router.post("/scheduler/trigger/etf")
async def trigger_etf_update(data_collector: DataCollectorService = Depends(get_data_collector)):
    """ETF 구성종목 업데이트 수동 실행"""
    try:
...
async def trigger_today_chart_update(data_collector: DataCollectorService = Depends(get_data_collector)):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"당일 차트 데이터 업데이트 트리거 실패: {str(e)}")


@admin_router.post("/scheduler/trigger/today-supply")
async def trigger_today_supply_demand_update(data_collector: DataCollectorService = Depends(get_data_collector)):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"당일 수급 데이터 업데이트 트리거 실패: {str(e)}")


@admin_router.post("/scheduler/trigger/adjustment-check")
async def trigger_adjustment_check(data_collector: DataCollectorService = Depends(get_data_collector)):
    """수정주가 체크 수동 실행"""
    try:
        logger.info("수동 수정주가 체크 실행")
        await data_collector.scheduler_service.trigger_adjustment_check_now()
        return {"message": "수정주가 체크가 완료되었습니다"}
    except Exception as e:
...
async def trigger_rs_update(data_collector: DataCollectorService = Depends(get_data_collector)):
    """RS 데이터 업데이트 수동 실행"""
    try:
        logger.info("수동 RS 데이터 업데이트 실행")
        await data_collector.scheduler_service.trigger_rs_update_now()
        return {"message": "RS 데이터 업데이트가 완료되었습니다"}
    except Exception as e:
...
async def get_adjustment_check_stats(data_collector: DataCollectorService = Depends(get_data_collector)):
    """수정주가 체크 통계 조회"""
    try:
        stats = data_collector.get_adjustment_check_stats()
        return {"adjustment_stats": stats, "status": "success"}
    except Exception as e:
...
async def check_adjustment_prices_for_stockai(
    batch_size: int = Query(100, description="배치 크기", ge=50, le=200), data_collector: DataCollectorService = Depends(get_data_collector)
):
    ...
    try:
        ...
        return {"message": "수정주가 체크가 완료되었습니다", "result": result, "status": "success"}
    except Exception as e:
...
async def clear_adjustment_cache(days_to_keep: int = Query(30, description="보관할 일수", ge=1, le=90), data_collector: DataCollectorService = Depends(get_data_collector)):
    """수정주가 캐시 정리"""
    try:
        data_collector.clear_old_adjustment_cache(days_to_keep=days_to_keep)
        return {"message": f"{days_to_keep}일 이전 수정주가 캐시가 정리되었습니다", "status": "success"}
    except Exception as e:
...
async def get_collection_stats(data_collector: DataCollectorService = Depends(get_data_collector)):
    """수집 통계 조회"""
    try:
...
async def update_symbol_mappings(data_collector: DataCollectorService = Depends(get_data_collector)):
    """종목 코드-이름 매핑 업데이트"""
    try:
        result = await data_collector.update_symbol_mappings()
        return {"message": "종목 매핑 업데이트 완료", "result": result, "status": "success"}
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"종목 매핑 업데이트 실패: {str(e)}")


@admin_router.post("/cache/refresh")
async def force_cache_refresh(data_collector: DataCollectorService = Depends(get_data_collector)):
    """캐시 강제 갱신"""
    try:
        await data_collector.force_cache_refresh()
        return {"message": "캐시 강제 갱신 완료", "status": "success"}
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"캐시 갱신 실패: {str(e)}")


@admin_router.get("/kiwoom/stats")
async def get_kiwoom_stats(data_collector: DataCollectorService = Depends(get_data_collector)):
    """키움 API 통계 조회"""
    try:
        stats = await data_collector.get_collection_stats()
        kiwoom_stats = stats.get("kiwoom_stats", {})

        return {"kiwoom_stats": kiwoom_stats, "status": "success"}
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"키움 API 통계 조회 실패: {str(e)}")


@admin_router.get("/etf/stats")
async def get_etf_crawler_stats(data_collector: DataCollectorService = Depends(get_data_collector)):
    """ETF 크롤러 통계 조회"""
    try:
        stats = await data_collector.get_collection_stats()
        etf_stats = stats.get("etf_stats", {})

        return {"etf_stats": etf_stats, "status": "success"}
    except Exception as e:
...
async def update_today_chart_data(batch_size: int = Query(100, description="배치 크기", ge=50, le=200), data_collector: DataCollectorService = Depends(get_data_collector)):
    """
    ...
    try:
        ...
        return {"success": True, "data": result, "message": result.get("message", "당일 차트 데이터 업데이트 완료")}
...
async def update_today_supply_demand_data(batch_size: int = Query(50, description="배치 크기", ge=20, le=100), data_collector: DataCollectorService = Depends(get_data_collector)):
    """
    당일 수급 데이터 업데이트
    ...
    try:
        ...
        return {"success": True, "data": result, "message": result.get("message", "당일 수급 데이터 업데이트 완료")}

    except Exception as e:
        logger.error(f"당일 수급 데이터 업데이트 실패: {e}")
        raise HTTPException(status_code=500, detail=f"당일 수급 데이터 업데이트 실패: {str(e)}")
...
async def start_batch_collection(
    ...
    try:
        ...
        # )
        result = await data_collector.collect_all_stock_chart_data(months_back=chart_months, batch_size=100, force_update=True)
        result = await data_collector.collect_all_supply_demand_data(months_back=chart_months, batch_size=100)
        return {"message": "대량 배치 수집 작업이 시작되었습니다", "result": result, "status": "success"}
    except Exception as e:
...
async def collect_chart_data_only(
    ...
    try:
        ...
        result = await data_collector.collect_all_stock_chart_data(months_back=months_back, batch_size=batch_size, force_update=force_update)

        return {"message": "차트 데이터 수집이 완료되었습니다", "result": result, "status": "success"}
    except Exception as e:
...
async def collect_supply_data_only(
    ...
    try:
        ...
        result = await data_collector.collect_all_supply_demand_data(months_back=months_back, batch_size=batch_size)

        return {"message": "수급 데이터 수집이 완료되었습니다", "result": result, "status": "success"}
    except Exception as e:
...
async def collect_single_stock_chart_data(
    ...
    try:
        ...
        if data_collector.settings.KIWOOM_APP_KEY != "test_api_key":
            chart_data = await data_collector.kiwoom_client.get_daily_chart_data(symbol, start_date_str, end_date_str)
        else:
            logger.warning("키움 API 키가 설정되지 않아 더미 데이터 사용")
            chart_data = []
        ...
        from stockeasy.collector.schemas.timescale_schemas import IntervalType, StockPriceCreate
        ...
        for chart_item in chart_data:
            try:
                ...
                api_volume_change_percent = (
                    safe_float(chart_item.volume_change_percent) if hasattr(chart_item, "volume_change_percent") and chart_item.volume_change_percent else None
                )
        ...
        if stock_price_data:
            ...
            if force_update:
                ...
                await timescale_service.delete_stock_prices_by_symbol_period(symbol, start_date, end_date)
            ...
            await timescale_service.bulk_create_stock_prices_with_progress(stock_price_data, batch_size=1000)
...
async def collect_single_stock_supply_data(
    ...
    try:
        ...
        # 키움 API에서 수급 데이터 조회
        if data_collector.settings.KIWOOM_APP_KEY != "test_api_key":
            supply_data = await data_collector.kiwoom_client.get_daily_supply_demand_data(symbol, start_date_str, end_date_str)
        else:
            logger.warning("키움 API 키가 설정되지 않아 더미 데이터 사용")
            supply_data = []

        if not supply_data:
            return {"message": f"종목 {symbol}의 수급 데이터가 없습니다", "symbol": symbol, "period": f"{start_date_str} ~ {end_date_str}", "total_records": 0, "status": "success"}
        ...
        if supply_demand_data:
            ...
            if force_update:
                # 강제 업데이트시 기존 데이터 삭제 후 재생성
                await timescale_service.delete_supply_demand_by_symbol_period(symbol, start_date, end_date)

            # 시간순으로 정렬하여 저장 (과거 → 현재)
            supply_demand_data.sort(key=lambda x: x.date)

            await timescale_service.bulk_create_supply_demand_with_progress(supply_demand_data, batch_size=1000)
...
async def get_batch_collection_status(data_collector: DataCollectorService = Depends(get_data_collector)):
    """배치 수집 상태 조회"""
    try:
        status = await data_collector.get_batch_collection_status()
        return {"batch_status": status, "status": "success"}
    except Exception as e:
...
async def get_timescale_health():
    ...
    try:
        ...
        return {"timescale_health": health, "status": "success"}
    except Exception as e:
...
async def get_timescale_stats():
    ...
    try:
        ...
        return {"timescale_stats": stats, "status": "success"}
    except Exception as e:
...
async def get_data_count_by_table(table_name: str, symbol: Optional[str] = Query(None, description="종목코드 (선택사항)")):
    """테이블별 데이터 건수 조회"""
    try:
        from stockeasy.collector.services.timescale_service import timescale_service
        ...
        return {"data_count": data_count, "status": "success"}
    except Exception as e:
...
async def check_duplicate_data(table_name: str, symbol: Optional[str] = Query(None, description="종목코드 (선택사항)")):
    """중복 데이터 체크"""
    try:
        from stockeasy.collector.services.timescale_service import timescale_service
        ...
        return {"duplicates": duplicates, "status": "success"}
    except Exception as e:
...
async def cleanup_old_data(table_name: str, days_to_keep: int = Query(730, description="보관할 일수", ge=30, le=3650)):
    """오래된 데이터 정리"""
    try:
        from stockeasy.collector.services.timescale_service import timescale_service
        ...
        return {"cleanup_result": result, "status": "success"}
    except Exception as e:
...
async def debug_supply_demand_data(symbol: str, limit: int = Query(10, description="조회할 데이터 수", ge=1, le=100)):
    """수급 데이터 디버깅용 조회"""
    try:
        ...
        async with get_timescale_session_context() as session:
            ...
            return {
                ...
                "date_range": {"min_date": str(date_range.min_date) if date_range else None, "max_date": str(date_range.max_date) if date_range else None} if date_range else None,
                "sample_data": [
                    {
                        "date": str(row.date),
                        "symbol": row.symbol,
...
async def get_all_rs_data(
    ...
    try:
        ...
        if gzip_enabled:
            ...
            return Response(content=compressed_content, media_type="application/json", headers={"Content-Encoding": "gzip", "Content-Length": str(len(compressed_content))})
        else:
            return response_data
...
async def get_rs_data_by_code(stock_code: str):
    """
    ...
    try:
        ...
        if rs_data is None:
            return SingleRSDataResponse(stock_code=stock_code, data=None, status="not_found", message=f"종목 {stock_code}의 RS 데이터를 찾을 수 없습니다")

        return SingleRSDataResponse(stock_code=stock_code, data=rs_data, status="success", message="RS 데이터 조회 성공")
...
async def update_rs_data(force_update: bool = Query(True, description="강제 업데이트 여부")):
    """
    ...
    try:
        ...
        return RSUpdateResponse(message=result["message"], updated_count=result["updated_count"], last_updated=result["last_updated"], status=result["status"])
...
async def collect_sector_chart_data(
    ...
    try:
        ...
        if sector_symbol.upper() not in ["KOSPI", "KOSDAQ"]:
            raise HTTPException(status_code=400, detail="지원되지 않는 업종입니다. KOSPI 또는 KOSDAQ만 지원됩니다.")

        logger.info(f"업종 {sector_symbol} 차트 데이터 수집 시작: {months_back}개월, 강제업데이트={force_update}")

        result = await data_collector.collect_sector_chart_data(sector_symbol=sector_symbol.upper(), months_back=months_back, force_update=force_update)
...
async def collect_all_sector_chart_data(
    ...
    try:
        ...
        result = await data_collector.collect_all_sector_chart_data(months_back=months_back, force_update=force_update)
...
async def get_sector_chart_data(
    ...
    try:
        ...
        chart_data = await data_collector.get_chart_data(symbol=sector_symbol.upper(), period=period, interval=interval, compressed=compressed)
...
async def batch_calculate_all_stocks(
    ...
    try:
        ...
        # 전종목 배치 계산 실행
        result = await timescale_service.batch_calculate_stock_price_changes(symbols=symbols, days_back=days_back, batch_size=batch_size)

        return {"message": "전종목 배치 계산이 완료되었습니다", "result": result, "status": "success"}
    except Exception as e:
...
async def batch_calculate_specific_symbols(
    ...
    try:
        ...
        # 특정 종목들 배치 계산 실행
        result = await timescale_service.batch_calculate_stock_price_changes(symbols=symbol_list, days_back=days_back, batch_size=batch_size)

        return {"message": f"특정 종목들({len(symbol_list)}개) 배치 계산이 완료되었습니다", "symbols": symbol_list, "result": result, "status": "success"}
    except HTTPException:
        raise
    except Exception as e:
...
async def batch_calculate_single_symbol(
    symbol: str, days_back: int = Query(30, description="계산할 일수", ge=1, le=1000), data_collector: DataCollectorService = Depends(get_data_collector)
):
    """
    단일 종목 배치 계산
    ...
    try:
        ...
        # 단일 종목 배치 계산 실행
        result = await timescale_service.batch_calculate_stock_price_changes(symbols=[symbol], days_back=days_back, batch_size=1)

        return {"message": f"종목 {symbol} 배치 계산이 완료되었습니다", "symbol": symbol, "result": result, "status": "success"}
    except Exception as e:
    ...
    try:
        ...
        return {
            ...
            "calculation_info": {
                ...
                "features": ["기존 API 계산값 보존", "NULL 값만 새로 계산", "COALESCE 함수로 안전한 업데이트", "배치 단위 처리로 고성능"],
            },
            "status": "success",
        }
    except Exception as e:
