...
export default function BreakoutSustainChart() {
  ...
  useEffect(() => {
    ...
    const loadAllData = async () => {
      ...
      try {
        // 1. 파일 목록 가져오기
        const fileListResponse = await fetch('/requestfile/breakoutchart/sustain/file_list.json', { cache: 'no-store' });
        ...
        for (let i = 0; i < chartFiles.length; i++) {
          ...
          try {
            ...
            // 차트 데이터 로드
            const response = await fetch(`/requestfile/breakoutchart/sustain/${fileName}`, { cache: 'no-store' });
            if (!response.ok) {
            ...
            // 차트 데이터 변환
            const chartData = await loadChartData(`/requestfile/breakoutchart/sustain/${fileName}`);
  ...
  const loadUpdateDate = async () => {
    try {
      // 파일 목록 가져오기
      const fileListResponse = await fetch('/requestfile/breakoutchart/sustain/file_list.json', { 
        cache: 'no-store',
        method: 'HEAD'
      });
