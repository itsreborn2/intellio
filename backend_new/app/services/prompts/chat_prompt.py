...
1. 기본 구조화:
   - 모든 응답은 적절한 제목(h2)과 소제목(h3)을 사용하여 구조화하세요
   - 중요한 수치나 키워드는 반드시 **굵은 글씨**로 강조하세요
   - 목록이나 단계가 필요한 경우 순서가 있는 목록(1., 2., ...)이나 순서 없는 불렛포인트를 사용하세요

2. 단락 구분:
   - 각 주제나 내용이 바뀔 때마다 새로운 단락으로 구분하세요
   - 연관된 내용은 같은 단락에서 설명하세요
   - 단락 간에는 적절한 여백을 유지하세요

3. 강조와 하이라이트:
   - 핵심 내용이나 중요한 결론은 **굵은 글씨**로 강조하세요
   - 특별히 주목해야 할 변화나 차이는 `코드 형식`으로 표시하세요
   - 중요한 수치나 비율은 **굵은 글씨**와 함께 구체적인 단위를 표기하세요
   - 첨언이 필요한 경우나 주석, 첨삭이 필요한 경우 또는 필요하다고 느낄때 이탤릭체를 사용하세요.

4. 표와 목록 활용:
   - 데이터를 비교할 때는 마크다운 표를 사용하세요
   - 여러 항목을 나열할 때는 순서 있는/없는 목록을 활용하세요
   - 복잡한 정보는 계층적 목록으로 구조화하세요

5. 결론 제시:
   - 분석 내용중 결론이 필요하다면 "## 결론" 섹션에서 명확하게 요약하세요
   - 핵심 시사점이나 권장사항이 있다면 굵게 강조하여 제시하세요
"""

        # 분석 유형별 추가 지침
        type_specific_format = {
            AnalysisType.TIME_SERIES: """
- 시간 순서대로 변화를 설명하고, 주요 변화 시점은 **굵게** 강조
- 증감률이나 변화량은 구체적인 수치와 함께 표시""",
...
- 주요 차이점은 **굵게** 강조하여 설명""",
            ...
            AnalysisType.RANKING: """
- 순위 정보는 순서 있는 목록으로 표시
- 상위/하위 항목의 특징은 **굵게** 강조""",

            AnalysisType.PATTERN: """
- 발견된 패턴을 구조화된 목록으로 제시
- 중요한 패턴이나 예외는 **굵게** 강조""",
...
- 핵심 내용을 섹션별로 구분하여 요약
- 중요한 결론이나 시사점은 **굵게** 강조
- "## 결론" 섹션에서 전체 내용을 통합적으로 정리"""
        }

        format_str = base_format
        for analysis_type in analysis_types:
            if analysis_type in type_specific_format:
                format_str += f"\n\n{analysis_type.value} 분석 시 추가 지침:{type_specific_format[analysis_type]}"
