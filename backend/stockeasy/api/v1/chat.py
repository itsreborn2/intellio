...
def get_user_friendly_agent_message(agent: str, status: str) -> str:
    ...
    """
    agent_messages = {
        "session_manager": {"start": "세션 초기화 중...", "complete": "세션 초기화 완료"},
        "orchestrator": {"start": "질문 분석 전략 수립 중...", "complete": "분석 계획 수립 완료"},
        "question_analyzer": {"start": "질문 의도 파악 중...", "complete": "질문 분석 완료"},
        "web_search": {"start": "웹 검색 중...", "complete": "웹 검색 완료"},
        "telegram_retriever": {"start": "내부 데이터 정보 검색 중...", "complete": "내부 데이터 검색 완료"},
        "report_analyzer": {"start": "기업 보고서 분석 중...", "complete": "기업 보고서 분석 완료"},
        "financial_analyzer": {"start": "재무 데이터 분석 중...", "complete": "재무 분석 완료"},
        "revenue_breakdown": {"start": "재무 데이터 분석 중...", "complete": "재무 데이터 분석 완료"},
        "industry_analyzer": {"start": "산업 및 경쟁사 분석 중...", "complete": "산업 분석 완료"},
        "confidential_analyzer": {"start": "비공개 정보 분석 중...", "complete": "비공개 정보 분석 완료"},
        "technical_analyzer": {"start": "기술적 분석 중...", "complete": "기술적 분석 완료"},
        "knowledge_integrator": {"start": "수집된 정보 통합 중...", "complete": "정보 통합 완료"},
        "summarizer": {"start": "결과 정리 중...", "complete": "결과 정리 완료"},
        "response_formatter": {"start": "답변 생성 중...", "complete": "답변 생성 완료"},
        "fallback_manager": {"start": "대체 정보 탐색 중...", "complete": "대체 정보 탐색 완료"},
        "parallel_search": {"start": "다중 데이터 소스 검색 시작...", "complete": "데이터 검색 완료"},
        "context_response": {"start": "문서 내용 검색 중...", "complete": "문서 내용 검색 완료"},
    }

    # 기본 메시지 (에이전트가 맵핑에 없는 경우)
    default_messages = {"start": f"{agent} 에이전트가 데이터 분석을 시작합니다.", "complete": f"{agent} 에이전트의 데이터 분석이 완료되었습니다."}
...
async def create_chat_session(
    request: ChatSessionCreateRequest, db: AsyncSession = Depends(get_db_async), current_session: Session = Depends(get_current_session)
) -> ChatSessionResponse:
    """새 채팅 세션을 생성합니다."""
    try:
        session_data = await ChatService.create_chat_session(
            db=db, user_id=current_session.user_id, title=request.title, stock_code=request.stock_code, stock_name=request.stock_name, stock_info=request.stock_info
        )
...
async def get_chat_sessions(
    ...
    try:
        ...
        result = await ChatService.get_chat_sessions(db=db, user_id=current_session.user_id, is_active=is_active, limit=limit, offset=offset)
        end_time = time.time()
        logger.info(
            f"[GET_CHAT_SESSIONS] 채팅 세션 목록 조회 완료: 소요 시간={end_time - start_time:.2f}초, 총 세션 수={result['total']}, 조회된 세션 수={len(result['sessions'])}"
        )

        return ChatSessionListResponse(
            ok=True, status_message="채팅 세션 목록 조회 완료", sessions=[ChatSessionResponse(**session) for session in result["sessions"]], total=result["total"]
        )

    except Exception as e:
        logger.error(f"채팅 세션 목록 조회 중 오류 발생: {str(e)}", exc_info=True)
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"채팅 세션 목록 조회 중 오류 발생: {str(e)}")


@chat_router.get("/sessions/{chat_session_id}", response_model=ChatSessionResponse)
async def get_chat_session(
    chat_session_id: UUID = Path(..., description="채팅 세션 ID"), db: AsyncSession = Depends(get_db_async), current_session: Session = Depends(get_current_session)
) -> ChatSessionResponse:
    """특정 채팅 세션의 상세 정보를 조회합니다."""
    try:
        session_data = await ChatService.get_chat_session(db=db, session_id=chat_session_id, user_id=current_session.user_id)
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"채팅 세션 상세 조회 중 오류 발생: {str(e)}")
...
async def update_chat_session(
    ...
    try:
        ...
        session_data = await ChatService.update_chat_session(db=db, session_id=chat_session_id, user_id=current_session.user_id, update_data=update_data)
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"채팅 세션 업데이트 중 오류 발생: {str(e)}")
...
async def delete_chat_session(
    ...
    """채팅 세션을 삭제합니다."""
    try:
        success = await ChatService.delete_chat_session(db=db, session_id=chat_session_id, user_id=current_session.user_id)
        ...
        return BaseResponse(ok=True, status_message="채팅 세션이 성공적으로 삭제되었습니다.")

    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"채팅 세션 삭제 중 오류 발생: {str(e)}")
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"채팅 세션 삭제 중 오류 발생: {str(e)}")
...
async def get_chat_messages(
    ...
    try:
        ...
        # 메시지 목록 조회
        messages = await ChatService.get_chat_messages(db=db, session_id=chat_session_id, user_id=current_session.user_id, limit=limit, offset=offset)
        ...
        return ChatMessageListResponse(messages=[ChatMessageResponse(**message) for message in messages], total=len(messages), ok=True, status_message="채팅 메시지 목록 조회 완료")

    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"채팅 메시지 목록 조회 중 오류 발생: {str(e)}")
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"채팅 메시지 목록 조회 중 오류 발생: {str(e)}")
...
async def stream_chat_message(
    ...
    """
    logger.info("[STREAM_CHAT] 스트리밍 메시지 처리 시작: 채팅 세션 ID={}, 메시지='{}', 후속질문={}", chat_session_id, request.message, request.is_follow_up)
    try:
        ...
        async with AsyncSessionLocal() as db:
            ...
            if not request.is_follow_up:
                ...
                messages = await ChatService.get_chat_messages(db=db, session_id=chat_session_id, user_id=current_session.user_id, limit=1, offset=0)
                ll = len(messages)
            ...
            agent_results = {}
            if is_follow_up:
                agent_results = await ChatService.get_chat_session_agent_results(db=db, session_id=chat_session_id, user_id=current_session.user_id)

                logger.info(f"[STREAM_CHAT] 후속질문 모드 세션 에이전트 결과: {agent_results.get('report_analyzer', {})}")

        logger.info("초기 데이터 조회 완료 및 DB 세션 자동 종료")
                        ...
                        if "components" in data:
                            for component in data["components"]:
                                if (
                                    ...
                                    and "data" in component
                                    and component["data"]
                                    and "chart_data" in component["data"]
                                    and component["data"]["chart_data"]
                                    and "dates" in component["data"]["chart_data"]
                                ):
        ...
        async def streaming_callback(chunk: str):
            ...
            # 청크 내용을 큐에 추가
            await streaming_queue.put(
                json.dumps({"event": "token", "data": {"token": chunk, "message_id": str(assistant_message_id), "timestamp": time.time()}}, cls=DateTimeEncoder)
            )
            # 전체 응답에 누적
            full_response += chunk
        ...
        async def preliminary_chart_callback(components: List[Dict[str, Any]], message: str, elapsed_time: float = 0.0, stock_info: Optional[Dict[str, Any]] = None):
            ...
            try:
                ...
                # JSON 직렬화 전 최종 검증
                logger.info("[STREAM_CHAT] preliminary_chart 이벤트 전송 시작")

                await streaming_queue.put(json.dumps(event_data, cls=DateTimeEncoder))

                logger.info("[STREAM_CHAT] preliminary_chart 이벤트 전송 완료")
                return True
        ...
        async def event_generator():
            ...
            # 서비스에서 사용할 DB 세션 생성
            rag_db = None
            stock_rag_service = None

            try:
                # 초기 이벤트 전송
                yield json.dumps({"event": "start", "data": {"message": "질문 처리를 시작합니다.", "timestamp": time.time()}})
                ...
                # 새 DB 세션 생성 및 RAG 서비스 초기화
                rag_db = await get_db_session()
                stock_rag_service = StockRAGService(db=rag_db)
                            ...
                            if state and "processing_status" in state:
                                ...
                                for agent, status in processing_status.items():
                                    if agent not in prev_status or prev_status[agent] != status:
                                        ...
                                        # 상태 변경 이벤트 전송
                                        event_data = {"agent": agent, "status": status, "timestamp": time.time(), "elapsed": elapsed}

                                        # 이벤트를 직접 yield하지 않고 큐에 전송
                                        await streaming_queue.put(json.dumps({"event": "agent_status", "data": event_data}, cls=DateTimeEncoder))
                                        ...
                                        if status == "processing":
                                            ...
                                            await streaming_queue.put(
                                                json.dumps(
                                                    {
                                                        "event": "agent_start",
                                                        "data": {"agent": agent, "message": user_friendly_message, "timestamp": time.time(), "elapsed": elapsed},
                                                    },
                                                    cls=DateTimeEncoder,
                                                )
                                            )
                ...
                try:
                    ...
                    if not answer or answer == "처리가 완료되었으나 응답을 찾을 수 없습니다.":
                        ...
                        logger.info(
                            f"[STREAM_CHAT] answer 키가 없어서 대안 사용: summary={bool(result.get('summary'))}, formatted_response={bool(result.get('formatted_response'))}"
                        )
                    ...
                    logger.info("[STREAM_CHAT] 응답 생성 완료 (총 소요시간: {:.2f}초, 응답 길이: {}자)", total_time, len(answer) if answer else 0)

                    # 메모리에 AI 응답 추가
                    await chat_memory_manager.add_ai_message(str(chat_session_id), answer)

                    # 메타데이터 설정
                    metadata = {"processing_time": total_time, "agents_used": list(prev_status.keys()) if prev_status else [], "responseId": str(uuid4())}
                    ...
                    async with AsyncSessionLocal() as save_db:
                        ...
                        if not is_follow_up:
                            ...
                            # 세션 업데이트
                            await ChatService.update_chat_session(db=save_db, session_id=chat_session_id, user_id=current_session.user_id, update_data=update_data)
                        ...
                        # 완료 이벤트 JSON 생성
                        complete_data = json.dumps({"event": "complete", "data": response_dict}, cls=DateTimeEncoder)
                    ...
                    error_data = json.dumps(
                        {"event": "error", "data": {"message": error_message, "timestamp": time.time(), "elapsed": time.time() - start_time}}, cls=DateTimeEncoder
                    )
                    yield f"data: {error_data}\n\n"

            finally:
                # RAG 서비스 정리
                if stock_rag_service:
                    try:
                        await stock_rag_service.close()
                        logger.info("[STREAM_CHAT] RAG 서비스 리소스 정리 완료")
                    except Exception as e:
                        logger.error(f"[STREAM_CHAT] RAG 서비스 정리 중 오류: {str(e)}")
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"채팅 메시지 스트리밍 중 오류 발생: {str(e)}")
    finally:
        gc.collect()
...
async def clear_chat_memory(
    ...
    try:
        ...
        if not success:
            raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="대화 메모리 초기화 중 오류가 발생했습니다.")

        logger.info(f"채팅 세션 {chat_session_id}의 메모리가 초기화되었습니다.")

        return BaseResponse(ok=True, status_message="채팅 세션의 메모리가 성공적으로 초기화되었습니다.")

    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"대화 메모리 초기화 중 오류 발생: {str(e)}")
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"대화 메모리 초기화 중 오류 발생: {str(e)}")
...
async def get_chat_memory(
    ...
    try:
        ...
        return ChatMemoryResponse(ok=True, status_message="채팅 세션의 메모리 조회가 완료되었습니다.", messages=messages, total=len(messages))

    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"대화 메모리 조회 중 오류 발생: {str(e)}")
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"대화 메모리 조회 중 오류 발생: {str(e)}")
...
async def save_chat_to_pdf(
    ...
    try:
        ...
        logger.info("PDF 생성 서비스 초기화 완료")
        # PDF 생성
        pdf_result = await pdf_service.generate_chat_pdf(chat_session=session_data, messages=messages, user_id=str(current_session.user_id), expert_mode=pdf_request.expert_mode)

        logger.info(f"PDF 생성 완료: {pdf_result['file_name']}")

        return PDFResponse(ok=True, status_message="PDF 생성 완료", download_url=pdf_result["download_url"], file_name=pdf_result["file_name"], expires_at=pdf_result["expires_at"])

    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"PDF 생성 중 오류 발생: {str(e)}", exc_info=True)
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"PDF 생성 중 오류 발생: {str(e)}")
...
async def create_share_link(
    ...
    """
    # 세션 소유권 확인
    query = select(StockChatSession).where(StockChatSession.id == chat_session_id, StockChatSession.user_id == current_session.user_id)
    result = await db.execute(query)
    session = result.scalar_one_or_none()

    if not session:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="채팅 세션을 찾을 수 없거나 접근 권한이 없습니다.")
...
async def get_shared_chat(share_uuid: str = Path(..., description="공유 UUID"), db: AsyncSession = Depends(get_db_async)):
    """공유된 채팅 세션 조회 API
