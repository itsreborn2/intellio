...
from common.services.token_usage_service import TokenUsageQueue, track_token_usage_bg, track_token_usage_sync
...
class LLMModels:
    ...
    def initialize(self, llm_list: list, **kwargs):
        ...
        if self._llm_chain is None:
            ...
            self._llm_streaming = self.get_llm(model_info["model"], model_info["api_key"], streaming=True, callback_handler=callback_handler, **kwargs)
    ...
    def get_llm(self, model_name: str, api_key: str | None, streaming: bool = False, callback_handler: Optional[BaseCallbackHandler] = None, **kwargs) -> BaseChatModel:
        if model_name == "openai":
            ...
            return ChatGoogleGenerativeAI(
                ...
                model="models/gemini-2.5-flash-lite-preview-06-17",  # 실험모델은 분당 횟수 제한이 심함 분당 2~5회. 사용불가한 수준
                google_api_key=api_key,
                temperature=kwargs.get("temperature", 0.1),
                max_output_tokens=kwargs.get("max_output_tokens", 20000),
                top_p=kwargs.get("top_p", 0.7),
            ...
            try:
                credentials = service_account.Credentials.from_service_account_file(
                    settings.GOOGLE_APPLICATION_CREDENTIALS_VERTEXAI, scopes=["https://www.googleapis.com/auth/cloud-platform"]
                )
    ...
    def get_current_llm_info(self) -> dict:
        ...
        return {"model": self._llm_list[self._current_model_idx]["model"], "current_index": self._current_model_idx, "total_models": len(self._llm_list)}
    ...
    def generate(self, user_query: str, prompt_context: str, user_id: Optional[UUID] = None, project_type: Optional[str] = None) -> Optional[AIMessage]:
        ...
        try:
            ...
            if user_id and project_type:
                ...
                with track_token_usage_sync(user_id=user_id, project_type=project_type, token_type="llm", model_name=self._llm.model, db_getter=SessionLocal) as tracker:
                    # API 호출 및 전체 응답 받기
                    ...
                    if hasattr(full_response, "usage_metadata"):
                        ...
                        tracker.add_tokens(
                            prompt_tokens=usage.get("input_tokens", 0), completion_tokens=usage.get("output_tokens", 0), total_tokens=usage.get("total_tokens", None)
                        )
    ...
    async def _save_token_usage_bg(self, user_id: UUID, project_type: str, token_type: str, model_name: str, token_data: dict):
        ...
        try:
            ...
            await queue.add_usage(user_id=user_id, project_type=project_type, token_type=token_type, model_name=model_name, token_data=token_data)
