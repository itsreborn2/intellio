...
4. 사업부문별 매출 현황
  - 기본 단위는 억원입니다.
  - 테이블의 단위가 백만원인 경우, 억원 단위로 변환하세요.
  - 단위 변환 시, 변환 과정을 상세히 기록하세요.
5. 제품별 매출 현황
6. 지역별 매출 현황
...
- 모든 분기의 값은 누적값이며, 이 값을 그대로 사용합니다.

**단위 변환 정확도 향상을 위한 지침:**
- 입력 데이터가 **백만원** 단위일 때, **억원** 단위로 변환하는 공식: **백만원 ÷ 100 = 억원**

**백만원 → 억원 변환 예시:**
- 13,254 백만원 ÷ 100 = 132.5억원
- 627,178 백만원 ÷ 100 = 6,272억원
- 5,000 백만원 ÷ 100 = 50억원

**기타 단위 변환 예시:**
- 1,200 십억원 ÷ 100 = 1.2조원
- 10,000 백만원 ÷ 100 = 100억원

**중요 주의사항:**
- 백만원을 억원으로 변환할 때는 **÷ 100**만 하면 됩니다.
- 곱셈(×)을 사용하지 마세요. 나눗셈(÷)만 사용하세요.
- 단위 변환 시 소수점 둘째 자리까지 표시하세요.

**💭 단위 변환 시 생각 과정 기록 필수:**
- 단위 변환을 할 때마다 반드시 다음 형식으로 계산 과정을 기록하세요:

```
[단위 변환 과정]
- 원본 데이터: 13,254 (백만원)
- 적용 공식: 백만원 ÷ 100 = 억원
- 계산 과정: 13,254 ÷ 100 = 132.54
- 최종 결과: 132.54억원
```

- 여러 값을 변환할 때는 각각의 변환 과정을 명시하세요.
- 변환된 값이 제대로 이루어졌는지 다시 한번 검증하세요.
