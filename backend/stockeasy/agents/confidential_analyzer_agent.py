...
from common.services.retrievers.semantic import (
    SemanticRetriever,
    SemanticRetrieverConfig,
)
from common.utils.util import async_retry
from stockeasy.agents.base import BaseAgent
from stockeasy.models.agent_io import ConfidentialData, RetrievedAllAgentData
from stockeasy.prompts.confidential_prompts import (
    CONFIDENTIAL_ANALYSIS_SYSTEM_PROMPT,
    CONFIDENTIAL_ANALYSIS_USER_PROMPT,
    format_confidential_contents,
)


# 테스트용 린터 경고 발생 코드
def test_linter_warnings():
    1 + 2 + 3 + 4 + 5  # E225: missing whitespace around operator
    if True:
        pass  # E701: multiple statements on one line
    return
...
class ConfidentialAnalyzerAgent(BaseAgent):
    ...
    def __init__(self, name: str | None = None, db: AsyncSession | None = None):
        ...
        logger.info(
            f"ConfidentialAnalyzerAgent initialized with provider: {self.agent_llm.get_provider()}, model: {self.agent_llm.get_model_name()}"
        )
    ...
    async def process(self, state: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            search_query = self._make_search_query(
                query, stock_code, stock_name, classification, state
            )
            ...
            metadata_filter = self._create_metadata_filter(
                stock_code, stock_name, classification, state
            )

            # 기업리포트 검색
            reports: list[ConfidentialData] = await self._search_reports(
                search_query, k, threshold, metadata_filter, user_id=user_id
            )
            ...
            if not reports:
                ...
                # 상태 업데이트 - 콜백 함수 사용
                if "update_processing_status" in state and "agent_name" in state:
                    state["update_processing_status"](
                        state["agent_name"], "completed_no_data"
                    )
                else:
                    # 기존 방식으로 상태 업데이트 (콜백 함수가 없는 경우)
                    state["processing_status"] = state.get("processing_status", {})
                ...
                logger.info(
                    f"ConfidentialAnalyzerAgent completed in {duration:.2f} seconds, found 0 reports"
                )
                return state
            ...
            try:
                ...
                if custom_prompt_from_state:
                    ...
                    logger.info(
                        f"ConfidentialAnalyzerAgent using custom prompt from state : {custom_prompt_from_state}"
                    )
                elif custom_prompt_from_attr:
                    system_prompt = custom_prompt_from_attr
                    logger.info(
                        "ConfidentialAnalyzerAgent using custom prompt from attribute"
                    )
            ...
            retrieved_data = cast(RetrievedAllAgentData, state["retrieved_data"])
            error_reports = []
            retrieved_data[self.retrieved_str] = error_reports
            ...
            logger.info(
                f"ConfidentialAnalyzerAgent processing_status: {state['processing_status']}"
            )
            ...
            logger.info(
                f"ConfidentialAnalyzerAgent completed in {duration:.2f} seconds, found {len(processed_reports)} reports"
            )
            return state
    ...
    def _create_metadata_filter(
        ...
        if state and "keywords" in state:
            time_keywords = [
                ...
                if any(t in kw for t in ["년", "분기", "월", "일", "최근"])
            ]
    ...
    def _parse_time_range(self, time_range: str) -> dict[str, str] | None:
        """
    ...
    async def _search_reports(
        ...
        try:
            ...
            if self.vs_manager is None:
                logger.debug(
                    "글로벌 캐시에서 VectorStoreManager 가져오기 시작 (ConfidentialAnalyzer)"
                )

                # 글로벌 캐시 함수를 직접 사용
                from stockeasy.graph.agent_registry import (
                    get_cached_vector_store_manager,
                )
                ...
                logger.debug(
                    "글로벌 캐시에서 VectorStoreManager 가져오기 완료 (ConfidentialAnalyzer)"
                )
            ...
            try:
                ...
                logger.warning(
                    f"[비공개자료 검색] retriever 실행 중 오류 발생: {str(e)}"
                )
                raise
            finally:
            ...
            for i, doc in enumerate(retrieval_result.documents):
                ...
                if i < 2:
                    ...
                    logger.debug(
                        f"문서 {i} 점수: {score:.3f}, 내용({len(content)}): {short_content}"
                    )
                    short_metadata = {
                        k: v
    ...
    def _process_reports(
        self, reports: list[ConfidentialData]
    ) -> list[ConfidentialData]:
        """
        검색된 리포트 처리
        ...
        for report in reports:
            ...
            if heading:
                ...
                processed_report["title"] = self._extract_title_from_content(
                    cleaned_content
                )
    ...
    async def _generate_report_analysis(
        ...
        try:
            ...
            # )
            result = await self.agent_llm.ainvoke_with_fallback(
